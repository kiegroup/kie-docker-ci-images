...
./update-jpa-config.sh $JBOSS_HOME/standalone/deployments/kie-drools-wb.war/WEB-INF/classes/META-INF/persistence.xml
...
./standalone.sh -b $JBOSS_BIND_ADDRESS -Dactivemq.artemis.client.global.thread.pool.max.size=30\
 -Djboss.kie.connection_url=$KIE_CONNECTION_URL -Djboss.kie.driver="$KIE_CONNECTION_DRIVER"\
 -Djboss.kie.username="$KIE_CONNECTION_USER" -Djboss.kie.password="$KIE_CONNECTION_PASSWORD" --server-config=standalone-full-kie-drools-wb.xml
exit $?